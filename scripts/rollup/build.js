...
function getPlugins(
  ...
  const isUMDBundle = false // xx
  //   bundleType === UMD_DEV ||
  //   bundleType === UMD_PROD ||
  //   bundleType === UMD_PROFILING;
  const isFBWWWBundle = false // xx
  //   bundleType === FB_WWW_DEV ||
  //   bundleType === FB_WWW_PROD ||
  //   bundleType === FB_WWW_PROFILING;
  const isRNBundle = false // xx
...
async function createBundle(bundle, bundleType) {
  ...
  // const isFBRNBundle = false // xx
    // bundleType === RN_FB_DEV ||
    // bundleType === RN_FB_PROD ||
    // bundleType === RN_FB_PROFILING;

  let resolvedEntry = resolveEntryFork(require.resolve(bundle.entry)); // xx

  const shouldBundleDependencies = false // xx
    // bundleType === UMD_DEV ||
    // bundleType === UMD_PROD ||
    // bundleType === UMD_PROFILING;
...
async function buildEverything() {
  ...
  for (const bundle of Bundles.bundles) { // xx
    bundles.push(
      // [bundle, NODE_ES2015],
      // [bundle, NODE_ESM],
      // [bundle, UMD_DEV],
  ...
  // await Packaging.copyAllShims(); // xx
