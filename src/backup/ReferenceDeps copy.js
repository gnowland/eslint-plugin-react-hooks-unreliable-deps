/** 🦑 @STATUS: At this point the function is working as desired. Code cleanup is in progress.
 * success: >>  177:22  warning  React Hook useCallback has an object in its dependency array: 'queryParameters'. Non-primitives can cause unnecessary renders due to reference equality (not value equality. Either use a property accessor to get its primitive value or convert the useCallback to a useDeepCompareCallback  react-hooks-unreliable-deps/reference-deps
 * @TODO: :1285 Remove the old rules to avoid re-warning. Move the Object ID logic into collectRecommendations (use dependencies?)
 * -------> need to remove the *collection* of potential dependencies and simply read what dependencies *exist*
 */
  ...
  create(context) {
    ...
    function visitFunctionWithDependencies(
      ...
      if (declaredDependenciesNode.type !== 'ArrayExpression') {
        ...
        /** 🦑 @TODO: How can I prevent warning user twice for same issue if eslint-plugin-react-hooks
         * is enabled too ? Can I somehow verify if exhaustive-deps is already enabled?
         */
        reportProblem({
        ...
        declaredDependenciesNode.elements.forEach(declaredDependencyNode => {
          ...
          /** 🦑 🌼🌼🌼 If we see an object then add a special warning. 🌼🌼🌼 */
          if (declaredDependencyNode.type === 'Identifier') {
            reportProblem({
              node: declaredDependencyNode,
              message:
                `React Hook ${context.getSource(reactiveHook)} has an object in its ` +
                `dependency array: '${declaredDependencyNode.name}'. Non-primitives ` +
                "can cause unnecessary renders from referencial equality checking. " +
                "Either use a property accessor to reveal its primitive value or " +
                `convert the ${context.getSource(reactiveHook)} to a ${context.getSource(reactiveHook).replace('use', 'useDeepCompare')}.`,
              // destructure the 'props' object outside of ` +
      ...
      /** 🦑 @NOTE: we're not worried about problemCount 👇 (duplicateDependencies, missingDependencies, or unnecessaryDependencies) */
      //      if (problemCount === 0) {
        // If nothing else to report, check if some dependencies would
        // invalidate on every render (constructons).
      ...
      /** 🦑 ✏️✏️✏️ >>>
       * HERE: e.g. React Hook useCallback has a missing dependency: 'query.address'. Either include it or remove the dependency array.
       */
      ...
      //        let badRef = null;
      //        Array.from(unnecessaryDependencies.keys()).forEach(key => { // js-ts
      //          if (badRef !== null) {
      //            return;
      //          }
      ...
      /** 🦑 HERE: THE WARNING IS CONSTRUCTED 🔮 */
      reportProblem({
        node: declaredDependenciesNode,
        message:
    ...
    /**
     * Visitor for call expressions (Hooks).
     */
    function visitCallExpression(node) {
      ...
      /**
       * 🦑 CHECK FOR OBJECT IN DECLAREDEPENDENCIESNODE HERE!!!!!??!?!?!?!
       */

      /** 🦑 I don't care about looking for dependencies in the callback */
      ...
      /** 🦑 I'm not concerned with no dependencies existing */
...
function collectRecommendations({
  ...
  /** 🦑 SHOULD I PUT MY OBJECT CHECK IN HERE??? 🤔🤔🤔 */
  declaredDependencies.forEach(({key}) => {
...
/** 🦑 Should I just add objects to this list? */
    ...
    .map(({key}) => {
      ...
      if (
        ...
        if (constantExpressionType != null) {
          console.log('🦑', constantExpressionType);
...
function getNodeWithoutReactNamespace(node) { // js-ts
  if (
    node.type === 'MemberExpression' &&
